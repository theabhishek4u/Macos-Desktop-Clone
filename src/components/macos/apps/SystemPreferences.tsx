                ...
                <div
                  className={`w-[72px] h-[48px] rounded-lg border-2 transition-all ${
                    mode === m
                      ? 'border-blue-500 shadow-md shadow-blue-500/20'
                      : 'border-gray-200 hover:border-gray-300'
                  } ${m === 'light' ? 'bg-white' : 'bg-gray-800'}`}
                >
                  <div className={`h-3 rounded-t-lg ${m === 'light' ? 'bg-gray-100' : 'bg-gray-700'}`} />
                </div>
...
function PlaceholderPane({ name, emoji }: { name: string; emoji: string }) {
  return (
...
function DetailView({ paneId }: { paneId: PaneId }) {
  ...
  switch (paneId) {
    ...
    case 'users':
      return <UsersPane />
    default:
      return <PlaceholderPane name={pane?.name ?? ''} emoji={pane?.emoji ?? '⚙️'} />
  }
}
      ...
      <div className="shrink-0 bg-[#f5f5f7]/90 backdrop-blur-md border-b border-gray-200/60 px-4 py-2.5 flex items-center gap-3">
        {selectedPane ? (
          <>
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-6 h-6 rounded-md ${currentPane?.color ?? 'bg-gray-500'} flex items-center justify-center text-[12px] shadow-sm`}
              >
                {currentPane?.emoji}
              </div>
          ...
          <div className="p-5">
            <div className="grid grid-cols-3 gap-4 max-w-[600px] mx-auto">
              {filteredPanes.map((pane) => (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-2xl ${pane.color} flex items-center justify-center text-[22px] shadow-md`}
                  >
                    {pane.emoji}
                  </div>
